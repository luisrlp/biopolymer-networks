...
gfortran -o anl_ai.o anl_ai.f90 umat_anl_ai.f90
find . -type f -path '*src/*' -name '_global.*' -exec cat {} +> test_in_abaqus/umat_anl_ai.f
find . -type f -path '*src/*' -name '*.f90' -not \( -name 'GETOUTDIR.f90' -o -name '_global.*' \) -exec cat {} >> test_in_abaqus/umat_anl_ai.f \;
